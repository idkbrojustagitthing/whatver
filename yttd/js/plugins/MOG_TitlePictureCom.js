 ...
 * @default -10  
 * 
 * @param Com_Count_shiftY
 * @desc コマンドが5個以上の場合、コマンド全体のY座標を(増えた数 * 数値pixel分)ずらします。
 * @default 25
 *
 * @param Command Pos 1
...
Scene_Title.prototype.create_picture_commands = function () {
	...
	for (i = 0; i < this._commandWindow._list.length; i++) {
		var symbol = this._commandWindow._list[i].symbol;
		var id = 0;
		switch (symbol) {
			case "continue": id = 1; break;
			case "nameGame2": id = 2; break;
			case "options": id = 3; break;
			case "miniEpisode": id = 4; break;
			case "credit": id = 5; break;
		}
		this._com_pictures.push(ImageManager.loadTitle2("Command_" + id));
...
Scene_Title.prototype.refresh_picture_command = function () {
	...
	var param = parseInt(Moghunter.parameters['Com_Count_shiftY'], 10);
	var addY = 0;
	if (param > 0) {
		var commandcount = this._commandWindow._list.length;
		addY = -(param) * (commandcount - 4);
	}
	for (i = 0; i < this._com_sprites.length; i++) {
		...
		this._com_sprites[i].y = this.cpsx[1] + addY;
